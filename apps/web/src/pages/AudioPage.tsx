...
const darkNightSpotlight = {
  ...
  description: "Artiste engage, spirituel et panafricain. Selection integree pour ecouter ses morceaux dans MAATFEED.",
  tracks: [
    { id: "XDOgZstP3fI", title: "C'est mon drapeau", label: "Derniere sortie" },
    { id: "3qm_nTRF1EQ", title: "Pardon Maman", label: "EP L'Eveil" },
    { id: "LV5Cwh70BD8", title: "Pardon Maman", label: "Short" },
    { id: "24UUYnTJ8XY", title: "Mon Peuple", label: "Short" },
    { id: "zaaowjipeGM", title: "10 February 2026", label: "Publication" },
    { id: "CH5bNpuR0So", title: "Fier d'etre Kamite", label: "Short" },
    { id: "tJC0Bs4ItRY", title: "Avec J.C", label: "Lyrics" },
    { id: "OmmF5pHsT3I", title: "Mon peuple", label: "Lyrics" },
    { id: "wQwOin2dMY8", title: "Grace a Dieu version longue", label: "Lyrics" },
    { id: "d3c24QyUWeg", title: "Pardon maman", label: "Lyrics" },
    { id: "xx6louj3UhM", title: "Je suis Kamite", label: "Lyrics" },
    { id: "YRcIZ75cWKg", title: "Plus jamais", label: "Lyrics" },
    { id: "P3bw1D8-Nfk", title: "L'eveil spirituel nous separe des gens qu'on aime", label: "Reflexion" },
    { id: "rI6YNiRbZCw", title: "Justice pour le diable", label: "Short" },
    { id: "G-hV_M783tg", title: "J'veux pas aller au paradis", label: "Morceau" }
  ]
};
...
function AudioSectionCard({
  ...
  mark?: AudioMark;
  onMark: (trackId: string, mark: AudioMark) => void;
}) {
    ...
    <article className="group min-w-[18rem] max-w-[18rem] overflow-hidden rounded-[1.5rem] border border-white/10 bg-black/25 shadow-xl shadow-black/30">
      ...
      <div className="space-y-4 p-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            type="button"
            onClick={() => onMark(track.id, "kept")}
            className={`rounded-full border px-3 py-2 text-sm transition-colors ${
              mark === "kept"
                ? "border-emerald-300/30 bg-emerald-400/12 text-emerald-100"
                : "border-white/10 text-sand/72 hover:border-white/20 hover:text-white"
            }`}
          >
            Ajouter
          </button>
          <button
            type="button"
            onClick={() => onMark(track.id, "review")}
            className={`rounded-full border px-3 py-2 text-sm transition-colors ${
              mark === "review"
...
export default function AudioPage() {
  ...
  const { currentTrack, isPlaying, playTrack, pause, seekTo } = useAudioPlayer();
  ...
  const [selectedDarkNightVideoId, setSelectedDarkNightVideoId] = useState(darkNightSpotlight.tracks[0].id);
  ...
  const reviewTracks = tracks.filter((track) => audioMarks[track.id] === "review").slice(0, 4);
  const selectedDarkNightTrack =
    darkNightSpotlight.tracks.find((track) => track.id === selectedDarkNightVideoId) ??
    darkNightSpotlight.tracks[0];
    ...
    <>
      ...
      <section className="px-4 py-6 pb-24">
        ...
        >
          ...
          <div className="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-2xl">
              <p className="mb-3 text-[11px] uppercase tracking-[0.24em] text-gold/80">Audio</p>
            <h1 className="font-display text-3xl text-gold lg:text-5xl">À écouter maintenant</h1>
            </div>

            <div className="grid grid-cols-2 gap-3 text-sm lg:min-w-[22rem]">
              <div className="rounded-[1.2rem] border border-white/10 bg-white/5 p-4">
                <p className="text-sand/60">Pistes</p>
                <p className="mt-2 text-2xl font-semibold text-white">{discovery?.sourceBlueprint.stats.curatedTrackCount ?? tracks.length}</p>
              </div>
              <div className="rounded-[1.2rem] border border-white/10 bg-white/5 p-4">
                <p className="text-sand/60">Avec contexte</p>
        ...
        <motion.section
          ...
          className="mt-6 overflow-hidden rounded-[1.4rem] border border-white/10 bg-black/24 shadow-xl shadow-black/24"
        >
          <div className="grid gap-0 lg:grid-cols-[0.95fr_1.25fr]">
            <div className="space-y-5 p-5">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="text-[11px] uppercase tracking-[0.18em] text-gold/75">Selection artiste</p>
                  <h2 className="mt-2 text-2xl font-semibold text-white">{darkNightSpotlight.artist}</h2>
                  <p className="mt-1 text-sm text-sand/58">{darkNightSpotlight.handle}</p>
                </div>
                <a
                  href={darkNightSpotlight.channelUrl}
                  target="_blank"
                  rel="noreferrer"
                  className="rounded-full border border-white/12 px-4 py-2 text-sm text-sand/82 transition-colors hover:border-white/24 hover:text-white"
                >
                  Ouvrir la chaine
                </a>
              </div>

              <p className="text-sm leading-6 text-sand/72">{darkNightSpotlight.description}</p>

              <div className="max-h-[24rem] space-y-2 overflow-y-auto pr-1">
                {darkNightSpotlight.tracks.map((track) => (
                  <button
                    key={track.id}
                    type="button"
                    onClick={() => {
                      pause();
                      setSelectedDarkNightVideoId(track.id);
                    }}
                    className={`w-full rounded-xl border px-3 py-3 text-left transition-colors ${
                      selectedDarkNightVideoId === track.id
                        ? "border-gold/40 bg-gold/12 text-white"
                        : "border-white/10 bg-white/[0.03] text-sand/78 hover:border-white/20 hover:text-white"
                    }`}
                  >
                    <span className="line-clamp-2 text-sm font-semibold">{track.title}</span>
                    <span className="mt-1 block text-xs uppercase tracking-[0.14em] text-sand/50">{track.label}</span>
                  </button>
                ))}
              </div>
            </div>

            <div className="border-t border-white/10 bg-black/22 p-3 lg:border-l lg:border-t-0">
              <iframe
                key={selectedDarkNightVideoId}
                src={`https://www.youtube.com/embed/${selectedDarkNightVideoId}?autoplay=0&controls=1&playsinline=1&rel=0&cc_lang_pref=fr&hl=fr&iv_load_policy=3&enablejsapi=1&origin=${encodeURIComponent(window.location.origin)}`}
                title={`Dark Night - ${selectedDarkNightTrack.title}`}
        ...
        {reviewTracks.length > 0 && (
          <motion.section
            ...
            className="mt-6 rounded-[1.2rem] border border-amber-300/15 bg-amber-400/[0.06] p-4"
          >
            <div className="mb-3 flex items-center justify-between gap-3">
              <h2 className="text-lg font-semibold text-white">Plus tard</h2>
              <span className="text-sm text-amber-100/70">{reviewTracks.length}</span>
            </div>
            <div className="grid gap-2 md:grid-cols-2">
              {reviewTracks.map((track) => (
                <button
                  ...
                  className="rounded-lg border border-white/10 bg-black/18 px-3 py-2 text-left text-sm text-sand/78 transition hover:bg-black/28 hover:text-white"
                >
                  <span className="line-clamp-1 font-medium text-white">{track.shortTitle || track.title}</span>
                  <span className="mt-1 block text-xs text-sand/55">
                  {track.highlightMoments?.[0]?.label || "Depuis le début"}
                  </span>
                </button>
              ))}
            </div>
              ...
              <div className="relative min-h-[20rem] overflow-hidden">
                ...
                <div className="relative flex h-full flex-col justify-end p-6">
                  ...
                  <div className="mt-5 flex flex-wrap gap-2">
                    ...
                    <button
                      type="button"
                      onClick={() => handleMarkTrack(featuredTrack.id, "kept")}
                      className={`rounded-full border px-5 py-3 text-sm transition-colors ${
                        audioMarks[featuredTrack.id] === "kept"
                          ? "border-emerald-300/30 bg-emerald-400/12 text-emerald-100"
                    ...
                    <button
                      type="button"
                      onClick={() => handleMarkTrack(featuredTrack.id, "review")}
                      className={`rounded-full border px-5 py-3 text-sm transition-colors ${
                        audioMarks[featuredTrack.id] === "review"
                          ? "border-amber-300/30 bg-amber-400/12 text-amber-100"
                          : "border-white/15 text-white hover:border-white/30"
                      }`}
                    >
              ...
              <div className="flex gap-4 overflow-x-auto pb-2">
                {section.tracks.map((track) => (
                  <AudioSectionCard
                    ...
                    mark={audioMarks[track.id]}
                    onMark={handleMarkTrack}
                  />
                ))}
              </div>
            </motion.section>
        ...
        {profile && (
          ...
          >
            <div className="mb-4 flex items-center justify-between">
              <div>
                <h2 className="text-2xl font-semibold text-gold">Tes listes</h2>
              </div>
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              {userPlaylists.length === 0 && (
                <div className="rounded-[1.2rem] border border-white/10 bg-black/20 p-4 text-sm leading-6 text-sand/72">
                  Aucune liste personnelle pour l&apos;instant.
                </div>
              )}
            </div>
          </motion.section>
        ...
        >
          <div className="mb-4">
            <h2 className="text-2xl font-semibold text-gold">Catalogue</h2>
          </div>
