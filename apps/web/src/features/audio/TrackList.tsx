...
import { deleteAudioTrack } from "../../services/adminService";
...
export function TrackList({ tracks, isLoading = false }: TrackListProps) {
  ...
  const handleDelete = (track: AudioTrack, event: React.MouseEvent) => {
  ...
  const confirmDelete = async () => {
    if (!isAdmin || !selectedTrack) return;
    ...
    try {
      ...
      showToast("Piste audio supprimée avec succès", "success");
      
      // Optionally trigger a refresh of the track list
      // This would need to be implemented based on your track list refresh strategy
      
    } catch (error) {
      console.error("Failed to delete audio track:", error);
      showToast("Échec de la suppression de la piste audio", "error");
    } finally {
      setIsDeleting(false);
    }
  };
  ...
  return (
    <>
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                ...
                {isAdmin && (
                  <button
                    ...
                    title="Supprimer la piste audio (admin)"
                  >
      ...
      <DeleteConfirmDialog
        ...
        description={`Êtes-vous sûr de vouloir supprimer "${selectedTrack?.title}" ? Cette action masquera la piste audio aux utilisateurs mais la conservera dans la base de données.`}
